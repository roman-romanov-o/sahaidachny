...
1. **Interface First** - Define types, protocols, and data models from code changes
...
4. **Read code changes**: Check `{task_path}/code-changes/` for interface definitions
...
- What interfaces need to be defined (from code changes)
- What tests need to be written (from test specs)
- What the acceptance criteria are

## TDD Implementation Process

### Phase 1: Interface Definition (from Code Changes)

**Goal**: Create type-safe contracts that tests and implementations will use.

1. **Read code changes** at `{task_path}/code-changes/`
...
**Example - From Code Change to Interface:**

```python
# From code-changes/users.md:
# POST /api/v1/users
...
**Interface issues**:
- Update interfaces to match code changes
...
- **blocked**: Cannot proceed (e.g., missing code changes or test specs)

### Common Issues

1. **No code changes found**
   - Report as blocked
...
4. **Conflicting code change and test spec**
   - Follow code change for interface
   - Note the conflict
...
{
  "status": "blocked",
  "summary": "Cannot proceed - no code changes found for PaymentService",
  "notes": "Expected code-changes/payment.md but directory is empty",
  "next_steps": "Need code change definition before TDD can proceed"
}
```
